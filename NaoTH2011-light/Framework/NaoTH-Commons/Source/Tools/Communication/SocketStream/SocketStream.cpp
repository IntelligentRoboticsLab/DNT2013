...
void SocketStream::init(GSocket* s)
{
    if ( socket != NULL)
    {
        // Some sort of shared pointer. We don't want it anymore, so we decrease pointer count to it.
    ...
    if ( s != NULL)
    {
        // Some sort of shared pointer. We are using it, so we increase pointer count to it.
        g_object_ref(s);
        socket = s;
    }
    mRecvdLen = 0;
...
int SocketStream::recv(std::string& msg) throw(std::runtime_error)
{
    ...
    // Status is -1 if an error is detected.
    if (status < 0)
    {
        std::string errMsg = err->message;
        g_error_free(err);
    ...
    // Otherwise status is number of bytes read from socket.
    else if (status > 0) {
        msg = mRecvBuf;
    }
    else
...
int SocketStream::prefixedRecv(std::string& msg)
{
    ...
    // Try to see whether we can extract the prefix.
